...
class GaussianModel:
    ...
    def create_pcd_from_image_render(self, cam_info, ext_points, init=False, scale=2.0, depthmap=None):
        cam = cam_info
        rgb_raw = (cam.original_image * 255).byte().permute(1, 2, 0).contiguous().cpu().numpy()
        rgb = o3d.geometry.Image(rgb_raw.astype(np.uint8))
        depth = o3d.geometry.Image(depthmap.astype(np.float32))

        return self.create_pcd_from_image_and_depth_render(cam, ext_points, rgb, depth, init)
    ...
    def create_pcd_from_image_and_depth_render(self, cam, ext_points, rgb, depth, init=False):
        if init:
            downsample_factor = self.config["dataset"]["pcd_downsample_init"]
        else:
            downsample_factor = self.config["dataset"]["pcd_downsample"]
    ...
    def extend_from_pcd_seq_render(
        ...
        fused_point_cloud, features, scales, rots, opacities = (
            self.create_pcd_from_image_render(cam_info, ext_points, init, scale=scale, depthmap=depthmap)
        )
        self.extend_from_pcd(
            fused_point_cloud, features, scales, rots, opacities, kf_id
        )
