...
class GaussianModel:
    ...
    def compute_3D_filter(self, cameras):
        ...
        for camera in cameras:
            ...
            R = camera.R.clone().detach().float()

            T = camera.T.clone().detach().float()
             # R is stored transposed due to 'glm' in CUDA code so we don't neet transopse here
            xyz_cam = xyz @ R.T + T[None, :]
    ...
    def create_pcd_from_image_and_depth(self, cam, rgb, depth, init=False):
        ...
        W2C = getWorld2View2(cam.R, cam.T).cpu().numpy()   #is C2W actually
        #time1 = time.time()
        ...
        #time12 = time.time()
        # print("create point cloud time is " , (time12 - time1)*1e3 , "ms")
    ...
    def create_pcd_from_image2(self, cam_info, ext_points , init=False, scale=2.0, depthmap=None):
        cam = cam_info
        image_ab = (torch.exp(cam.exposure_a)) * cam.original_image + cam.exposure_b
        image_ab = torch.clamp(image_ab, 0.0, 1.0)
        rgb_raw = (image_ab * 255).byte().permute(1, 2, 0).contiguous().cpu().numpy()
        if abs(sum(sum(depthmap))) < 1e-5:
        ...
        return self.create_pcd_from_image_and_depth2(cam, ext_points , rgb, depth, init)

    def create_pcd_from_image_and_depth2(self, cam, ext_points , rgb, depth, init=False):
        if init:
            downsample_factor = self.config["dataset"]["pcd_downsample_init"]
        else:
            downsample_factor = self.config["dataset"]["pcd_downsample"]
       ...
       # if init: #only down sample when init
        pcd_tmp = pcd_tmp.random_down_sample(1.0 / downsample_factor)
        new_xyz = np.asarray(pcd_tmp.points)
        new_rgb = np.asarray(pcd_tmp.colors)
        #time12 = time.time()
        ...
        # dist2 = (
        #     torch.clamp_min(
        #         distCUDA3(P1 , fused_point_cloud2),
        #         0.0000001,
        #     )
        #     * point_size
        # )
    ...
    def extend_from_pcd_seq2(
        self, cam_info, ext_points , kf_id=-1, init=False, scale=2.0, depthmap=None
    ):
        fused_point_cloud, features, scales, rots, opacities = (
            self.create_pcd_from_image2(cam_info, ext_points , init, scale=scale, depthmap=depthmap)
        )
    ...
    def reset_opacity_nonvisible(
        ...
        current_opacity_with_filter = self.get_opacity_with_3D_filter
        opacities_new = torch.min(current_opacity_with_filter, torch.ones_like(current_opacity_with_filter)*0.4)
    ...
    def densification_postfix(
        ...
        new_xyz_gradient_accum_abs = torch.zeros((new_xyz.shape[0], 1), device="cuda")
        new_xyz_gradient_accum_abs_max = torch.zeros((new_xyz.shape[0], 1), device="cuda")
        ...
        self.xyz_gradient_accum_abs = torch.cat((self.xyz_gradient_accum_abs, new_xyz_gradient_accum_abs))
        self.xyz_gradient_accum_abs_max = torch.cat((self.xyz_gradient_accum_abs_max, new_xyz_gradient_accum_abs_max))
    ...
    def add_densification_stats(self, viewspace_point_tensor, update_filter):
        self.xyz_gradient_accum[update_filter] += torch.norm(   #gradient accum
        ...
        self.denom[update_filter] += 1     #denom accum
