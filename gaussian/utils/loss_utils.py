...
def l1_loss_mask(image, image_med, water_mask, gt):

    l1_rgb = torch.abs(image_med * water_mask - gt * water_mask) + torch.abs(image * ~water_mask - gt * ~water_mask)
    return l1_rgb.mean()
...
def ssim(img1, img2, window_size=11, size_average=True):
    # Use fused_ssim for GPU-accelerated SSIM computation
    if img1.dim() == 3:
