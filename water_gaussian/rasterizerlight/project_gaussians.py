...
# from https://github.com/booqo/lowlight-underwater-for-nerf-studio/blob/master/lowlight_underwater/rasterizerlight/project_gaussians.py
...
def project_gaussians(
    ...
    theta: Float[Tensor, "1 3"],
    rho: Float[Tensor, "1 3"],
    fx: float,
    fy: float,
    cx: float,
    cy: float,
...
class _ProjectGaussians(Function):
    ...
    def backward(
        ...
        (v_cov2d, v_cov3d, v_mean3d, v_scale, v_quat, v_tau) = _C.project_gaussians_backward(
            ctx.num_points,
            means3d,  #pW
            scales,
            ctx.glob_scale,
        ...
        v_tau = torch.sum(v_tau, dim=0)
        v_rho = v_tau[:3].view(1, -1)
        v_theta = v_tau[3:].view(1, -1)
        ...
        return (
            ...
            # viewmat: Float[Tensor, "4 4"],
            v_viewmat,
            # vtheta: Float[Tensor, "1 3"],
            v_theta,
            # vrho: Float[Tensor, "1 3"],
            v_rho,
            # fx: float,
            None,
            # fy: float,
