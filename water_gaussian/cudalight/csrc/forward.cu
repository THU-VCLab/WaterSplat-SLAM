...
__global__ void nd_rasterize_forward( //实际的光栅化处理部分，它使用之前计算的高斯数据来决定每个像素的最终颜色
    const dim3 tile_bounds,
    const dim3 img_size,
    ...
    const float* __restrict__ medium_rgb, //c^med
    const float* __restrict__ medium_bs, //\sigma^bs
    const float* __restrict__ medium_attn, //\sigma^attn
    const float* __restrict__ depths, // s_i
    float* __restrict__ final_Ts,
    int* __restrict__ final_index,
    int* __restrict__ first_index,
    float* __restrict__ out_img, //C
    ...
    for (int b = 0; b < num_batches; ++b) {
        ...
        for (int t = 0; (t < batch_size) && !done; ++t) {
            ...
            #pragma unroll
            for (int c = 0; c < channels; ++c) {
                const float exp_obj = __expf(-medium_attn[channels * pix_id + c] * depth); // -\sigam_attn *s_i
                const float exp_bs = __expf(-medium_bs[channels * pix_id + c] * depth); // -\sigam_bs *s_i
                pix_out[c] = __hadd(pix_out[c], __float2half((exp_obj * colors[channels * g + c] + exp_bs * medium_rgb[channels * pix_id + c]) * vis));
            } // C         = exp_obj * colors[channels * g + c] *  \alpha *T             +        exp_bs * medium_rgb[channels * pix_id + c]   *     \alpha *T
            T = next_T;
...
__global__ void rasterize_forward(
    ...
    const float3* __restrict__ color_enhance,
    const float* __restrict__ depths,
    float* __restrict__ final_Ts,
    int* __restrict__ final_index,
    int* __restrict__ first_index,
    ...
    int2 range = tile_bins[tile_id];
    const int block_size = block.size();
    int num_batches = (range.y - range.x + block_size - 1) / block_size;  //batch size
    ...
    if (inside) {
        ...
        color_enhance_pix = color_enhance[pix_id];
        prev_depth = 0.f;
    ...
    for (int b = 0; b < num_batches; ++b) {
        ...
        for (int t = 0; (t < batch_size) && !done; ++t) {
            ...
            const float3 c_out = {vis * c.x * color_enhance_pix.x, vis * c.y * color_enhance_pix.y, vis * c.z * color_enhance_pix.z};
...
__device__ void project_cov3d_ewa(
    ...
    compensation = std::sqrt(std::max(0.000025f, det_orig / det_blur));
}
