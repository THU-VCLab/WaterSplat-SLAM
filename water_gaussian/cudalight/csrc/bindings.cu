...
project_gaussians_backward_tensor(
    ...
    project_gaussians_backward_kernel<<<
        ...
        (float *)v_tau.contiguous().data_ptr<float>()
    );

    return std::make_tuple(v_cov2d, v_cov3d, v_mean3d, v_scale, v_quat, v_tau);
}
...
rasterize_forward_tensor(
    ...
    const torch::Tensor &color_enhance,
    const torch::Tensor &depths,
    ...
    rasterize_forward<<<tile_bounds_dim3, block_dim3>>>(
        ...
        (float3 *)color_enhance.contiguous().data_ptr<float>(),
        depths.contiguous().data_ptr<float>(),
...
std::
    tuple<
        ...
        torch::Tensor, // dL_dmedium_bs
        torch::Tensor,  // dL_dmedium_attn
        torch::Tensor  // dL_dcolor_enhance
        >
    ...
    torch::Tensor v_color_enhance = torch::zeros({img_height, img_width, 3}, xys.options());
    ...
    rasterize_backward_kernel<<<tile_bounds, block>>>(
        ...
        *(float3 *)background.contiguous().data_ptr<float>(),
        ...
        (float3 *)v_color_enhance.contiguous().data_ptr<float>()
    );

    return std::make_tuple(v_xy, v_conic, v_colors, v_opacity, v_medium_rgb, v_medium_bs, v_medium_attn, v_color_enhance);
}
