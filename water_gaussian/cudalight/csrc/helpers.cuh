...
inline __device__ float3 transform_4x3_rot_only_transposed(const float *mat, const glm::vec3 p) {
    float3 out = {
        mat[0] * p.x + mat[4] * p.y + mat[8] * p.z,
        mat[1] * p.x + mat[5] * p.y + mat[9] * p.z,
        mat[2] * p.x + mat[6] * p.y + mat[10] * p.z,
...
inline __device__ glm::vec3 project_pix_vjp_glm(
    const float2 fxfy, const float3 p_view, const float2 v_xy
) {
    float rw = 1.f / (p_view.z + 1e-6f);
    float2 v_proj = { fxfy.x * v_xy.x, fxfy.y * v_xy.y };
...
inline __device__ glm::mat3 makeSkewSymmetric(const glm::vec3& v) {
    return glm::mat3(
        0.0f,  v.z,   -v.y,
        -v.z,   0.0f,  v.x,
        v.y,   -v.x,   0.0f
...
inline __device__ glm::mat3 makeSkewSymmetric(const float3& v) {
    return glm::mat3(
        0.0f,  v.z,   -v.y,
        -v.z,   0.0f,  v.x,
        v.y,   -v.x,   0.0f
