...
#     from water_splatting import csrc as _C
# except ImportError:
...
if cuda_toolkit_available():
    ...
    if os.path.exists(os.path.join(build_dir, "lowlight_underwater.so")) or os.path.exists(
        os.path.join(build_dir, "lowlight_underwater.lib")
    ):
        ...
        with Console().status(
            "[bold yellow]lowlight_underwater: Setting up CUDA (This may take a few minutes the first time)",
    ...
    Console().print(
        "[yellow]lowlight_underwater: No CUDA toolkit found. lowlight_underwater will be disabled.[/yellow]"
    )
