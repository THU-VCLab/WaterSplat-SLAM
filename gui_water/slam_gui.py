...
from gui_water.gl_render import util, util_gau
from gui_water.gl_render.render_ogl import OpenGLRenderer
from gui_water.gui_utils import (
    GaussianPacket,
    Packet_vis2main,
    create_frustum,
    cv_gl,
...
from mast3r_slam.network import MLP

o3d.utility.set_verbosity_level(o3d.utility.VerbosityLevel.Error)


class SLAM_GUI:
    def __init__(self, params_gui=None, embedded_dim=128, L_degree=4, layer_num=2):
        self.step = 0
        ...
        # hidden_dims = [embedded_dim for i in range(layer_num)]
        # self.Medium_MLP = MLP(input_dim=6*L_degree, output_dim=9, hidden_dims=hidden_dims, output_activation="sigmoid")
        # self.Light_MLP = MLP(input_dim=6*L_degree, output_dim=3, hidden_dims=hidden_dims, output_activation="sigmoid")
    ...
    def init_widget(self):
        ...
        self.window = gui.Application.instance.create_window(
            "WaterSplat-SLAM GUI", self.window_w, self.window_h
        )
        ...
        self.panel.add_child(gui.Label("Medium rendering options")) # add medium
        medium_chbox_tile_geometry = gui.Horiz(0.5 * em, gui.Margins(margin))

        self.with_medium_chbox = gui.Checkbox("With medium attn")
        self.with_medium_chbox.checked = False
        medium_chbox_tile_geometry.add_child(self.with_medium_chbox)

        self.medium_bs_chbox = gui.Checkbox("Medium bs")
        self.medium_bs_chbox.checked = False
        medium_chbox_tile_geometry.add_child(self.medium_bs_chbox)

        self.medium_attn_chbox = gui.Checkbox("Medium attn")
        self.medium_attn_chbox.checked = False
        medium_chbox_tile_geometry.add_child(self.medium_attn_chbox)

        self.medium_chbox = gui.Checkbox("Medium")
        self.medium_chbox.checked = False
        medium_chbox_tile_geometry.add_child(self.medium_chbox)

        self.medium_allClear_chbox = gui.Checkbox("Medium clear all")
        self.medium_allClear_chbox.checked = False
        medium_chbox_tile_geometry.add_child(self.medium_allClear_chbox)

        self.panel.add_child(medium_chbox_tile_geometry)
    ...
    def _on_screenshot_btn(self):
        ...
        # Save the full GUI view (3D scene + GS rendering background)
        img_gui = np.asarray(app.render_to_image(self.widget3d.scene, width, height))
        cv2.imwrite(f"{filename}-gui.png", cv2.cvtColor(img_gui, cv2.COLOR_RGB2BGR))

        # Save the GS rendering result
        if self.render_img is not None:
            img = np.asarray(self.render_img)
            cv2.imwrite(f"{filename}.png", cv2.cvtColor(img, cv2.COLOR_RGB2BGR))

        # Save transparent PNG using GS rendering opacity as alpha
        if self.render_img is not None and self.render_opacity is not None:
            render_rgb = np.asarray(self.render_img).astype(np.float32) / 255.0  # [H,W,3]

            # opacity shape: [1,H,W] or [3,H,W] -> [H,W,1]
            opacity = self.render_opacity.detach().cpu().numpy()
            if opacity.ndim == 3:
                opacity = opacity[0]  # [H,W] take first channel
            if opacity.ndim == 2:
                opacity = opacity[:, :, np.newaxis]  # [H,W,1]

            # Resize opacity to match render_img if needed
            rh, rw = render_rgb.shape[:2]
            oh, ow = opacity.shape[:2]
            if (oh, ow) != (rh, rw):
                opacity = cv2.resize(opacity, (rw, rh))[:, :, np.newaxis]

            alpha = np.clip(opacity, 0.0, 1.0)
            rgba = np.concatenate([render_rgb, alpha], axis=2)  # [H,W,4]
            rgba8 = (rgba * 255.0 + 0.5).astype(np.uint8)
            rgba8_bgra = cv2.cvtColor(rgba8, cv2.COLOR_RGBA2BGRA)
            cv2.imwrite(f"{filename}-transparent.png", rgba8_bgra)
    ...
    def render_o3d_image(self, results, current_cam):
        if self.depth_chbox.checked:
        ...
        elif self.with_medium_chbox.checked:
            rgb = (
                (torch.clamp(results["rgb_object"], min=0, max=1.0) * 255)
                .byte()
                .permute(1, 2, 0)
                .contiguous()
                .cpu()
        ...
        elif self.medium_chbox.checked:
            rgb = (
                (torch.clamp(results["rgb_medium"], min=0, max=1.0) * 255)
                .byte()
                .permute(1, 2, 0)
                .contiguous()
                .cpu()
        ...
        elif self.medium_bs_chbox.checked:
            rgb = (
                (torch.clamp(results["medium_bs"], min=0, max=1.0) * 255)
                .byte()
                .permute(1, 2, 0)
                .contiguous()
                .cpu()
        ...
        elif self.medium_attn_chbox.checked:
            rgb = (
                (torch.clamp(results["medium_attn"], min=0, max=1.0) * 255)
                .byte()
                .permute(1, 2, 0)
                .contiguous()
                .cpu()
            ...
            render_img = o3d.geometry.Image(rgb)
        elif self.medium_allClear_chbox.checked:
            rgb = (
                (torch.clamp(results["rgb_clear"], min=0, max=1.0) * 255)
                .byte()
                .permute(1, 2, 0)
                .contiguous()
                .cpu()
            ...
            render_img = o3d.geometry.Image(flipped_img)
            # cv2.flip(img, 0, img)
            ...
            rgb = (
                (torch.clamp(results["render"], min=0, max=1.0) * 255) # without attn and medium bs
                .byte()
                .permute(1, 2, 0)
                .contiguous()
                .cpu()
    ...
    def render_gui(self):
        ...
        self.render_opacity = results.get("opacity", None)
...
def run(params_gui=None, embedded_dim=128, L_degree=4, layer_num=2):
    app = o3d.visualization.gui.Application.instance
    app.initialize()
    win = SLAM_GUI(params_gui, embedded_dim=embedded_dim, L_degree=L_degree, layer_num=layer_num)
    app.run()
