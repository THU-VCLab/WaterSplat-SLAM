...
class FrameTracker:
    ...
    def track(self, frame: Frame, water_mask=True):
        keyframe = self.keyframes.last_keyframe()

        idx_f2k, valid_match_k, Xff, Cff, Qff, Xkf, Ckf, Qkf = mast3r_match_asymmetric(
            self.model, frame, keyframe, idx_i2j_init=self.idx_f2k   #get points in the current frame according to last keyframe
        )
        # Save idx for next
        self.idx_f2k = idx_f2k.clone()  # f ids related to k ids

        if water_mask:
            Qff = Qff*(~(frame.water_mask.cuda())).reshape(-1,1)
            Qkf = Qkf*(~(keyframe.water_mask.cuda())).reshape(-1,1)

        # Get rid of batch dim
        idx_f2k = idx_f2k[0]
        valid_match_k = valid_match_k[0]

        Qk = torch.sqrt(Qff[idx_f2k] * Qkf)  #keyframe conf
    ...
    def get_points_poses(self, frame, keyframe, idx_f2k, img_size, use_calib, K=None):
        ...
        if use_calib:
            Xf = constrain_points_to_ray(img_size, Xf[None], K).squeeze(0)  #back projection by depth and known calibration
            Xk = constrain_points_to_ray(img_size, Xk[None], K).squeeze(0)  #unormalized rays
    ...
    def solve(self, sqrt_info, r, J):
        ...
        mdim = J.shape[-1]
        A = (robust_sqrt_info[..., None] * J).view(-1, mdim)  # dr_dX  N*3,7
    ...
    def opt_pose_calib_sim3(
        ...
        sqrt_info = torch.cat((sqrt_info_pixel.repeat(1, 2), sqrt_info_depth), dim=1)  #N 3
        ...
        old_cost = float("inf")
        for step in range(self.cfg["max_iters"]):
            Xf_Ck, dXf_Ck_dT_CkCf = act_Sim3(T_CkCf, Xf, jacobian=True)  #trans Xff to Xkk using T_CkCf, dXf_Ck_dT_CkCf is [N , 3 , 7]
