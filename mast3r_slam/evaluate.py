...
from .colmap_utils import rotmat2qvec


def prepare_savedir(args, dataset):
    base_dir = config.get("output", {}).get("base_dir", "outputs/default")
    save_dir = pathlib.Path(base_dir)
...
def prepare_savedir_wt(dataset):
    seq_name = dataset.dataset_path.stem
    return seq_name
...
def save_traj_wt(
    ...
    with open(logfile, "w") as f:
        ...
        for t, keyframe in frames.items():
            R_mats = keyframe.T_WC[:3,:3].cpu().numpy().reshape(3, 3)
            T_vecs = keyframe.T_WC[:3,3].cpu().numpy().reshape(3)
            Q = rotmat2qvec(R_mats) # [qw, qx, qy, qz]
            x, y, z = T_vecs.data
            qw, qx, qy, qz = Q.data
